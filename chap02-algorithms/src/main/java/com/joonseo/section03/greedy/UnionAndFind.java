...
public class UnionAndFind {
    ...
    *  두 집합의 대표 원소를 비교하여 두 집합이 연결 되도록 한다. */
    ...
    public static String solution(String input) throws IOException {
        ...
        // parent 배열 초기화 ( 처음에는 가각ㄱ의 요소가 별동의 집합으로 구성 )
        parent = new int[N+1];
        for(int i = 1; i <= N; i++){
            parent[i] = i;
        }
        ...
        for(int i = 0; i<M; i++){
            ...
            union(a, b); // a학생과 b학생을 같은 집합으로 만든다.
            System.out.println("union(" + a + ", " + b + ")"); // 디버깅용 출력문
            System.out.println("parent : " + Arrays.toString(parent)); // 디버깅용 출력문
        }
