package com.joonseo.section08.graph;
...
public class UndirectedGraphAdjMatrix {
    ...
    private List<Boolean> active;  // 각 노드의 활성 상태 (true이면 노드가 존재) ( 실제로 삭제하지는 않음 )
    ...
    public void addEdge(int u, int v) {
        if (u < vertices && v < vertices && active.get(u) && active.get(v)) { // 정상범위와 활성화 상태인지 확인
            matrix[u][v] = true;
            matrix[v][u] = true;
        }
    }
    ...
    public void printGraph() {
        ...
        for (int i = 0; i < vertices; i++) {
            if (active.get(i)) { // 활성화 상태인것만
                for (int j = 0; j < vertices; j++) {
                    if (active.get(j)) { // 연결된 노드도 활성화 상태이면
